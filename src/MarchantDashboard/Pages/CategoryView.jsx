...
//     console.log(token)
//     const handleEdit = () =>{
//         axios.get(`https://newecomerceappbackend.onrender.com/api/v1/category/${id}`)
//         .then((res)=>{
//             console.log(res)
...
//     }
//   return (
//     <div className=' w-[100%] bg-[red] flex items-center justify-center h-[100%] '>
//         <div className=" w-[60%] h-[90%] flex items-center justify-center bg-[white] flex-col ">
...
//                 <div className="w-[100%] h-[30%] flex items-center justify-center  "></div>
//                 <div className=" w-[100%] h-[30%] flex items-center justify-center "></div>
//             </div>
//             <div className="w-[100%] h-[30%] flex items-center justify-center  ">
//                 <button>Edit</button>
...
const CategoryView = () => {
  ...
  const fetchCategory = async () => {
    try {
      const response = await axios.get(`https://new-ecomer-ce-app-backend2.vercel.app/api/v1/category/${id}`, {
        // headers: { Authorization: `Bearer ${token}` },
      });
      console.log(response.data.data.category)
      setCategory(response.data.data.category);
      setCategoryName(response.data.data.category.categoryName);
      setCategoryDescription(response.data.data.category.categoryDescription);
      setImagePreview(response.data.data.category.categoryImage);
    } catch (err) {
      console.error(err);
  ...
  const handleUpdate = async () => {
    ...
    try {
      setLoading(true);
      const response =  await axios.put(`https://new-ecomer-ce-app-backend2.vercel.app/api/v1/category/${id}`, formData, {
        headers: {
  ...
  const handleDelete = async () => {
    try {
        const response =  await axios.delete(`https://new-ecomer-ce-app-backend2.vercel.app/api/v1/category/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      console.log(response)
      ...
      <div className="w-full max-w-3xl h-[100%] items-center justify-center flex-col flex bg-white rounded-lg shadow-2xl p-6">
        {editMode ? (
          ...
          <div className='w-[70%] h-[100%]   flex items-center justify-center flex-col '>
            <h2 className="text-[30px] h-[30%] items-center flex justify-center font-bold mb-4">{category.categoryName}</h2>
